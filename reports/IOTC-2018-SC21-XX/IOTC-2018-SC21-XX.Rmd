...
- \fancyhead[R]{IOTC-2018-SC21-XX}
license: Creative Commons Attribution-ShareAlike 4.0 International Public License
bibliography: references.bib
subtitle: IOTC-2018-SC21-XX - 21th Session Indian Ocean Tuna Commission Scientific
  Committee
...
Table 1 below summarizes the grid of uncertanties considered for the conditioning of the OM. This initial grid results in a total of 2,592 model runs.
...
results[,c("ratio","SSB15SSB0","F15FMSY","SSB15SSBMSY"):=list(SSB_MSY/SSB_Virgin,SSB_endyr/SSB_Virgin,F_endyr/Fstd_MSY,SSB_endyr/SSB_MSY)]
...
Exploration of results shed some light on the levels of uncertainty in the grid, as well as on problematic  areas of the grid: combinations of parameters leading to unreasonable or surprising results. 

Conditioning of the albacore OM had to confront the problem of runs estimating very large values of virgin biomass to explain the observed catch levels and abundance trends when the imposed model parameters led to low stock productivity. The distribution of estimates for virgin biomass (SB0) in the current OM grid (Figure \ref{fig:densvb}) also show some models with very large estimates of virgin biomass, however on the lower end of the estimates the SB0 is considered reasonable.
...
Additionally, the same factors seem to be influent in the current stock status of SBcurrent/SB0 (Figure \ref{fig:depfactor}).
...
```{r compare, fig.cap="Estimates of virgin biomass (SB0) vs. SBcurrent (2015)/SB0 levels for all 2401 model runs. Darker blue represents model runs with SB0 bellow 450,000 and low SBcurrent (2015)/SB0 levels, while lighter blue represents model runs with SB0 above 450,000 and high SBcurrent (2015)/SB0 levels"}
...
It seems clear that the final grid of runs should not include those runs starting from large biomasses and that are close to virgin levels, despite the long history of exploitation, as they will not provide an useful testing ground on which to test management procedures for the Indian Ocean swordfish stock. The authors decided to filter out the runs were virgin biomass is high (above 450,000) and depletion is low (above 0.60). A total of 136 model runs met this criteria, therefore were excluded of OM.
...
The application of both selection criteria presented above left a total of 2265 model runs as part of the base case OM. Inspection of the effects of the different uncertanties considered were explored visually for SB0, SBcurrent/SBMSY and SBcurrent/SB0 (Figures \ref{fig:boxSB0}, \ref{fig:boxSBcurrSBMSY},\ref{fig:boxSBcurrSB0})
...
```{r boxSBcurrSBMSY, out.width = "80%", fig.cap=""}
...
```{r boxSBcurrSB0, out.width = "80%", fig.cap=""}
...
Additional inspection of the effects of the interactions between relevant uncertanties considered were explored visually for SBMSY/SB0, SB0 and SBcurrent/SB0 (Figures \ref{fig:distSBMSYSB0}, \ref{fig:distSB0},\ref{fig:distSBcurrSB0})


```{r distSBMSYSB0, out.width = "80%", fig.cap=""}
...
```{r distSB0, out.width = "80%", fig.cap=""}
ggplot(results, aes(SSB_Virgin)) + geom_density(aes(fill=factor(cpue)), alpha=0.3)+
...
```{r distSBcurrSB0, out.width = "80%", fig.cap=""}
...
<!-- ```{r filterom, fig.cap="Population trajectories (recruitment, SSB, catch and F) estimated by the filtered OM grid for Indian Ocean swordfish. The black line shows the median value, while the darker and lighter ribbons show the 50\\% and 90\\% quantiles, respectively."} -->
<!-- plot(oms) -->
<!-- ``` -->
...
A number of model runs appear to introduce unreasonable combinations of parameters and data, and thus lead to estimates of virgin biomass and SBcurrent/SB0 that should not be considered feasible. A subjective filter was applied to exclude these runs from the final OM.
