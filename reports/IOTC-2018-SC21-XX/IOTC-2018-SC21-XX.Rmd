...
abstract: |
  This document presents the current status of development of an Operating Model for the Indian Ocean swordfish (*Xiphias gladius*) stock. It explores the role of the structural uncertainty in the current stock assessment by means of a grid of SS3 model fits. The current grid results in 2592 alternative population trajectories and productivity estimates. Currently, a subset of 2265 model runs is proposed to compose the OM, these were explored regarding the effects on several indicators and residual analysis of CPUE indices and stock-recruitment relationship. A preliminary analysis was performed to identify clusters from which model runs can be resampled to reduce the number of runs in the OM, as currently 2265 runs are too computationally demanding.
---
...
```{r CPUEres, out.width = "80%",fig.cap="CPUE residuals for the 2265 runs for Indian Ocean swordfish. The black line shows the median value, while the darker and lighter ribbons show the 50\\% and 90\\% quantiles, respectively. From top to bottom panels represent residuals for: JPLL NW, JPLL NE, JPLL SW, JPLL SE, TWLL NW, TWLL NE, TWLL SW, TWLL SE, EU.POR SW"}
...
A preliminary clustering analysis was performed, using hierarchical clustering with virgin spawning biomass, spawning biomass at MSY, current spawning biomass and biomass at MSY as response variables. For testing purposes, the 2265 were grouped in 4 clusters following the results of the hierarchical cluster analysis (Figure \ref{fig:dendogram}) each with 378, 560, 966, 361 runs. Virgin spawning biomass, biomass at MSY and SBcurr/SB0 by cluster are presented in Figure \ref{fig:clusters}.

![Dendrogram of the cluster analysis results.\label{fig:dendogram}](figures/Dendogram.png){width=65%}

![Boxplot of virgin spawnig biomass (SB0), biomass at MSY (B0), and SBcurr/SB0 by cluster.\label{fig:clusters}](figures/Clustering_quantities.png){width=60%}
...
This document has introduced the operating model for the Indian Ocean stock of swordfish conditioning and inspecting. The OM is built by introducing multiple sources of structural uncertainty to the stock assessment model conducted by WPB [@WPB2017] and based on the Stock Synthesis 3 platform [@Methot2013SS3]. Currently, one element in the grid has not been correctly implemented, therefore results are still preliminary. When these runs have been implemented a similar approach to the described in this document will be used to explore the OM.

Currently, a number of model runs appear to introduce unreasonable combinations of parameters and data, and thus lead to estimates of virgin biomass and SBcurr/SB0 that should not be considered feasible. A subjective filter was applied to exclude these runs from the OM, resulting in 2265 runs being kept.
...
- Explore model runs through diagnostics;
